...
const FireBaseTools = {
   ...
   */
    fetchUser: () => {
        return new Promise((resolve, reject) => {
            const localUser = JSON.parse(localStorage.getItem(localStorage.key(0)));
            if (localUser.uid) {
                resolve(localUser);
            } else {
   ...
   */
    ...
    getUserData: (user) => {
        return new Promise((resolve, reject) => {
            if (user !== null) {
                firebaseDb.ref(`users/${user.uid}/coins/`).once('value', (snapshot) => {
    ...
    setUserData: (user, collections, collection, coinId, isCheked) => {
        return firebaseDb.ref(`users/${user.uid}/coins/${collections}/${collection}/`).update({[coinId]: isCheked}).then(() => {
            return [collections, collection, coinId, isCheked];
        });
    }
};
